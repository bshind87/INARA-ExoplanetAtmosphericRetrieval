...
# ------------------------------------------------------------------
DATASETS = {
    'PSG Spectra (3 503 samples)': {
        'data_dir':    Path('inara_data/processed_psg'),
        'results_dir': Path('results'),
        'label':       'PSG',
        'x_label':     'Wavelength (µm)',
        'x_type':      'spectra',
    },
    'INARA ATMOS — Full 124k': {
...
dataset_name = st.sidebar.selectbox('Dataset', list(DATASETS.keys()), index=1)
ds = DATASETS[dataset_name]
DATA_DIR    = ds['data_dir']
RESULTS_DIR = ds['results_dir']

if not DATA_DIR.exists() and ds['x_type'] == 'spectra':
    DATA_DIR = Path('inara_data/processed')
...
def metrics_bar_chart(bl_df, dl_df, metric='R2', split='Test'):
    mols = MOLECULE_NAMES
    ...
    if dl_vals is not None:
        fig.add_bar(name='SpectralResNet (124k)', x=mols, y=dl_vals,
...
if has_data:
    st.sidebar.metric('Samples loaded', f'{len(spectra):,}')
...
# ====================================================================
    ...
    def _mean_r2(df):
        ...
        return df[df['molecule'].isin(MOLECULE_NAMES)]['R2'].clip(lower=-1.5).mean()

    # --- Summary cards ---
    st.subheader('Overall Performance Summary')
    cols = st.columns(4)
    cols[0].metric('Baseline Val R²',    f"{_mean_r2(bl_val):.4f}"   if bl_val  is not None else 'N/A')
    cols[1].metric('Baseline Test R²',   f"{_mean_r2(bl_test):.4f}"  if bl_test is not None else 'N/A')
    cols[2].metric('SpectralResNet Val R²', f"{_mean_r2(dl_val):.4f}" if dl_val is not None else 'N/A')
    cols[3].metric(
        'SpectralResNet Test R²',
        f"{_mean_r2(dl_test):.4f}" if dl_test is not None else 'N/A',
        delta=(f"{_mean_r2(dl_test)-_mean_r2(bl_test):+.4f} vs baseline"
    ...
    with col_dl:
        st.markdown('**SpectralResNet** *(full 124k training samples)*')
    ...
    if bl_test is not None and dl_test is not None and split == 'Test':
        ...
        st.divider()
        st.subheader('R² Gain: SpectralResNet vs Baseline RF (Test Set)')
        ...
        fig_delta = apply_layout(fig_delta,
            title='ΔR² = SpectralResNet − Baseline RF  (positive = CNN advantage)',
...
# ====================================================================
    ...
    c2.metric('SpectralResNet R²',   f'{r2_dl:.4f}'  if not np.isnan(r2_dl) else 'N/A',
    ...
    c4.metric('ResNet MAE',          f'{mean_absolute_error(y_true, y_dl):.4f}' if y_dl is not None else 'N/A')
    ...
    with col_r:
        if y_dl is not None:
            ...
            st.plotly_chart(scatter_pred(y_true, y_dl_c, 'SpectralResNet'),
                            use_container_width=True)
        else:
    ...
    if dl_pred is not None:
        dl_r2_all = [max(_r2(targets[:, i], dl_pred[:, i]), -1.5)
                     if targets[:, i].var() > 1e-10 else float('nan')
                     for i in range(12)]
        rows.append(dl_r2_all); row_names.append('SpectralResNet')
